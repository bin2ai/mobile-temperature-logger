...
class SerialConnectApp:
    ...
    # GUI elements
    tk_root: tk.Tk
    tk_label_port: ttk.Label
    tk_combobox_port: ttk.Combobox
    tk_button_connect: ttk.Button
    tk_label_status: ttk.Label
    tk_timer: tk.Tk
    tk_label_timer: ttk.Label
    tk_button_timer: ttk.Button
    tk_label_delay: ttk.Label
    tk_slider_delay: ttk.Scale
    tk_label_period: ttk.Label
    tk_scale_period: ttk.Scale
    tk_label_samples: ttk.Label
    tk_button_reset: ttk.Button
    # Serial port
    ...
    def __init__(self, root: tk.Tk):
        self.tk_root = root
        self.tk_root.title("Mobile Temperature Logger (MTL)")
        ...
        self.tk_label_port = ttk.Label(root, text="Select COM Port:")
        self.tk_label_port.pack(padx=10, pady=5)

        self.tk_combobox_port = ttk.Combobox(root, state="readonly")
        self.tk_combobox_port.pack(padx=10, pady=5)

        # Bind the refresh_ports function to the <FocusIn> event of the combobox
        self.tk_combobox_port.bind("<FocusIn>", self.refresh_ports)

        self.tk_button_connect = ttk.Button(
            root, text="Connect", command=self.toggle_connection)
        self.tk_button_connect.pack(padx=10, pady=5)
        # Initially disable the button
        self.tk_button_connect["state"] = "disabled"

        self.tk_label_status = ttk.Label(root, text="Status: Not Connected")
        self.tk_label_status.pack(padx=10, pady=5)
        ...
        self.tk_label_delay = ttk.Label(root, text="Delay (seconds):")
        self.tk_label_delay.pack(padx=10, pady=5)
        self.tk_slider_delay = ttk.Scale(
            root, from_=0, to=255, orient=tk.HORIZONTAL, command=self.set_button_function)
        self.tk_slider_delay.pack(padx=10, pady=5)
        self.tk_slider_delay.set(60)
        self.tk_slider_delay.bind("<ButtonPress-1>", self.on_slider_drag_start)
        self.tk_slider_delay.bind(
            "<ButtonRelease-1>", self.on_slider_drag_stop)

        self.tk_label_period = ttk.Label(root, text="Period (seconds):")
        self.tk_label_period.pack(padx=10, pady=5)
        self.tk_scale_period = ttk.Scale(
            root, from_=1, to=100, orient=tk.HORIZONTAL, command=self.set_button_function)
        self.tk_scale_period.pack(padx=10, pady=5)
        self.tk_scale_period.set(60)
        self.tk_scale_period.bind("<ButtonPress-1>", self.on_slider_drag_start)
        # estimated time to finish text (period * 1000 + delay in seconds)
        self.tk_scale_period.bind(
            "<ButtonRelease-1>", self.on_slider_drag_stop)
        ...
        self.tk_button_reset = ttk.Button(
            root, text="Reset", command=self.reset_button_function)
        self.tk_button_reset.pack(padx=10, pady=5)
        ...
        self.tk_root.after(self.timer_interval, self.check_and_refresh)

        self.tk_root.after(5000, self.update_device_state)
    ...
    def update_device_state(self):
        ...
        if self.is_connected:
            try:
                ...
                # HH:MM:SS
                self.est_time_left_label[
                    "text"] = f"Estimated time left: {est_time_left//3600} Hr(s):{(est_time_left%3600)//60} Min(s):{est_time_left%60} Sec(s)"
        ...
        self.tk_root.after(self.timer_interval_device_state,
                           self.update_device_state)
        ...
        if not self.slider_dragging:
            ...
            try:
                self.serial_port.write(
                    b'AT+DELAY=' + str(self.tk_slider_delay.get()).encode() + b'\r\n')
                response = self.serial_port.readline().decode()
                if "OK" not in response:
                ...
                self.tk_label_delay["text"] = f"Delay: {delay} seconds"

                self.serial_port.write(
                    b'AT+PERIOD=' + str(self.tk_scale_period.get()).encode() + b'\r\n')
                response = self.serial_port.readline().decode()
                if "OK" not in response:
                ...
                self.tk_label_period["text"] = f"Period: {period} seconds"
    ...
    def update_state(self):
        if not self.is_connected:
            ...
            self.start_button["state"] = "disabled"
            self.tk_button_reset["state"] = "disabled"
            self.tk_slider_delay["state"] = "disabled"
            self.tk_scale_period["state"] = "disabled"
            self.download_button["state"] = "disabled"
        else:
            # enable start, set, and reset buttons and slides
            self.start_button["state"] = "normal"
            self.tk_button_reset["state"] = "normal"
            self.tk_slider_delay["state"] = "normal"
            self.tk_scale_period["state"] = "normal"
    ...
    def refresh_ports(self, event=None):
        ...
        self.tk_combobox_port["values"] = available_ports

    def check_and_refresh(self):
        # Check if the serial port is connected
        if self.serial_port and self.serial_port.is_open:
            self.tk_label_status["text"] = "Status: Connected"
            self.tk_button_connect["text"] = "Disconnect"
            # Enable the button for disconnecting
            self.tk_button_connect["state"] = "normal"
        else:
            self.tk_label_status["text"] = "Status: Not Connected"
            self.tk_button_connect["text"] = "Connect"
            # Enable the button for connecting
            self.tk_button_connect["state"] = "normal"

        # Check if the selected COM port is still available
        selected_port = self.tk_combobox_port.get()
        available_ports = [port.device for port in list_ports.comports()]
